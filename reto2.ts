...
function manufactureGifts(giftsToProduce: Toy[]): string[] {
  const result = giftsToProduce.filter((gift) => gift.quantity > 0).flatMap((gift) => Array(gift.quantity).fill(gift.toy));
  return result;
}