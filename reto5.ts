...
function timeUntilTakeOff(
  ...
  // All your code here
  const timestampFromTime = convertToTimestamp(fromTime);
  const timestamptakeOffTime = convertToTimestamp(takeOffTime);
  return Math.floor((timestamptakeOffTime - timestampFromTime) / 1000);
}

function convertToTimestamp(elfDateTime: string) {
  const val = elfDateTime.replace(" NP", "").split(/[*,@,|]/);
  const [year, month, day, hour, minute, second] = val.map((str) =>
    parseInt(str, 10)
  );
